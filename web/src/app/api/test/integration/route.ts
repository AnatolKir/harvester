...
export async function GET() {
  ...
  try {
    const { error } = await supabase.from("video").select("video_id").limit(1);
  ...
  if (testVideoId) {
    try {
      ...
      results.push({
        ...
        message: error
          ? error.message
          : `${data?.length || 0} comments created`,
        data: data,
      });
    } catch (error: any) {
      results.push({
  ...
  if (testVideoId) {
    try {
      ...
      await supabase.from("comment").delete().eq("video_id", testVideoId);

      // Delete test video
      await supabase.from("video").delete().eq("video_id", testVideoId);
