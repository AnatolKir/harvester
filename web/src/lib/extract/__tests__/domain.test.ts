...
describe("extractDomains", () => {
  ...
  it("handles protocol and www and path", () => {
    expect(extractDomains("http://www.example.com/path?x=1")).toEqual([
      "example.com",
    ]);
  });
  ...
  it("removes zero-width characters", () => {
    ...
    expect(extractDomains(`check ${zeroWidth}`)).toEqual(["example.com"]);
  });
  ...
  it("ignores bare dots or punctuation tokens", () => {
    expect(extractDomains("see [.], not a domain")).toEqual([]);
  });

  it("deduplicates repeated domains", () => {
    expect(extractDomains("example.com and https://example.com")).toEqual([
      "example.com",
    ]);
  });
});
