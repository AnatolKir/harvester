...
async function postToSlack(title: string, message: string, details?: Record<string, unknown>) {
  ...
  try {
    ...
    // Non-blocking alerting; ignore errors
  }
}
...
export const killSwitchJob = inngest.createFunction(
  ...
  async ({ event, step, logger }) => {
    ...
    try {
      ...
      // Step 3: Notify Slack (non-blocking)
      ...
      // Step 4: Cancel all pending scheduled jobs (if supported by Inngest)
  ...
  async ({ event, step, logger }) => {
    ...
    try {
      ...
      const alerts = await step.run('check-for-alerts', async () => {
        ...
        // Alert if DLQ backlog is high (acts as DLQ using failed jobs)
        if ((healthData as any).deadLetterQueueSize >= 10) {
          alerts.push({
            type: 'error',
            message: `Dead letter queue backlog: ${(healthData as any).deadLetterQueueSize}`,
          });
        }
      ...
      // Step 4: Send Slack alerts (non-blocking, feature-flagged)
      await step.run('send-slack-alerts', async () => {
        if (alerts.length > 0) {
          const title = `System Health Alerts (${alerts.length})`;
          const message = alerts
            .map((a: any) => `• ${String(a.type).toUpperCase()}: ${a.message}`)
            .join('\n');
          await postToSlack(title, message, {
            killSwitchActive: healthData.killSwitchActive,
            deadLetterQueueSize: (healthData as any).deadLetterQueueSize,
            successRate: (healthData as any).successRate,
          });
        }
      });
