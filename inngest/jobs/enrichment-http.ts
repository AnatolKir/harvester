...
async function isRobotsDisallowAll(domainUrl: string): Promise<boolean> {
  try {
    ...
    // naive check: if Disallow: / under User-agent: * then skip
...
export const httpEnrichmentJob = inngest.createFunction(
  ...
  async ({ step, logger, attempt }) => {
    ...
    // Step 2: Pick candidate domains (recent, missing http metadata)
    const candidates = await step.run('select-candidates', async () => {
      ...
      const out = (data || []).filter((d: any) => {
        ...
        // Treat as lacking if no http block or missing checked_at/verified flag
        return !httpMeta || !httpMeta.checked_at;
      });
      return out.slice(0, 60); // pre-trim for safety
    });

    let processed = 0;
    let updated = 0;

    // Step 3: Process each domain, respecting token bucket (30/min)
    for (const d of candidates) {
      if (processed >= 60) break; // bound work per run
      ...
      await step.run(`enrich-${d.id}`, async () => {
        ...
        // robots check (best-effort; skip if disallow all)
